...
# Navigation tabs
tab_rankings, tab_h2h, tab_gm = st.tabs([
    "🏆 Map Power Rankings",
...
with tab_h2h:
    ...
    with col_t1:
        ...
        if "h2h_team_a" in st.session_state and st.session_state["h2h_team_a"] not in teams_a_opts:
            st.session_state["h2h_team_a"] = teams_a_opts[def_a_idx]
    ...
    with col_t2:
        ...
        if "h2h_team_b" in st.session_state and st.session_state["h2h_team_b"] not in teams_b_opts:
            st.session_state["h2h_team_b"] = teams_b_opts[def_b_idx]
...
with tab_gm:
    ...
    **Abyss, Ascent, Haven, Lotus, Split, Summit, Sunset**.
    ...
    with col_sim_a:
        ...
        if "gm_a" in st.session_state and st.session_state["gm_a"] not in teams_gm_a:
            st.session_state["gm_a"] = teams_gm_a[def_gm_a_idx]
    ...
    with col_sim_b:
        ...
        if "gm_b" in st.session_state and st.session_state["gm_b"] not in teams_gm_b:
            st.session_state["gm_b"] = teams_gm_b[def_gm_b_idx]
