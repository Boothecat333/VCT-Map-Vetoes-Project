...
### 2. Exponential Recency Decay (60-Day, 75% Retention)
Matches smoothly decay over 60-day intervals, retaining 75% of their weight after 2 months:
$$w_i = (0.75)^{\frac{\Delta\text{days}}{60}}$$

### 3. Bayesian Shrinkage & Opponent Adjustment (Two-Degree Iteration)
Teams with small sample sizes on a map are regressed towards league baseline using a prior pseudo-weight $C = 2.5$:
...
Continuous ratings are converted into a full-spectrum 0–100 esports Power Score via logistic transformation centered at 50 ($k = 0.95$):
$$PS(T, m) = \frac{100}{1 + \exp(-0.95 \cdot R(T, m))}$$
- **85–100**: Elite Map Specialist (Dominant comfort pick)
- **65–84**: Strong Map Advantage
- **45–64**: Contested / Balanced Map
