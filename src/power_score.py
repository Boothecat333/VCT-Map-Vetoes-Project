...
DEFAULT_HALF_LIFE_DAYS = 45.0  # User-configured 45-day half life
DEFAULT_PRIOR_WEIGHT = 3.0     # Bayesian shrinkage pseudo-matches
...
class PowerScoreModel:
    ...
    def compute_recency_weight(self, match_date: date, ref_date: date) -> float:
        """Computes exponential decay weight: w = 2^(-delta_days / half_life)."""
        delta_days = (ref_date - match_date).days
        if delta_days <= 0:
            return 1.0
        return float(2.0 ** (-delta_days / self.half_life_days))
    ...
    def fit(self, df: pd.DataFrame) -> "PowerScoreModel":
        ...
        # Rating = 0 -> 50; Rating = +5 -> 77.7; Rating = -5 -> 22.3
        k = 0.25
        self.ratings = {}
        for (team, map_name), raw_r in curr_ratings.items():
            ps = 100.0 / (1.0 + np.exp(-k * raw_r))
