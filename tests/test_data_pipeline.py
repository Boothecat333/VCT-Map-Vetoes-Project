...
def test_teams_at_champions_2026():
    from src.database import get_teams_by_league, CHAMPIONS_2026_TEAMS
    leagues = get_teams_by_league()
    assert "Teams at Champions 2026" in leagues
    champs = leagues["Teams at Champions 2026"]
    assert len(champs) == 16
    expected = [
        "100 Thieves", "LOUD", "NRG", "G2 Esports",
        "TYLOO", "JD Gaming", "EDward Gaming", "Xi Lai Gaming",
        "Karmine Corp", "Team Liquid", "FUT Esports", "Team Vitality",
        "Global Esports", "Nongshim RedForce", "Paper Rex", "T1"
    ]
    for team in expected:
        assert team in champs
