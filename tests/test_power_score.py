...
def test_recency_weight_half_life():
    model = PowerScoreModel(half_life_days=45.0)
    ref_date = date(2026, 9, 18)
    
    # Same day should have weight 1.0
    assert model.compute_recency_weight(ref_date, ref_date) == 1.0
    
    # 45 days ago should have weight 0.5
    date_45d_ago = ref_date - timedelta(days=45)
    w_45 = model.compute_recency_weight(date_45d_ago, ref_date)
    assert pytest.approx(w_45, 0.01) == 0.5
    
    # 90 days ago should have weight 0.25
    date_90d_ago = ref_date - timedelta(days=90)
    w_90 = model.compute_recency_weight(date_90d_ago, ref_date)
    assert pytest.approx(w_90, 0.01) == 0.25
...
def test_model_fitting_and_power_scores():
    ...
    model = PowerScoreModel(half_life_days=45.0, reference_date=date(2026, 9, 1))
    model.fit(df)
